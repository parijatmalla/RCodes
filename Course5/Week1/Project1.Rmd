...
```{r cars}
	...
	file<-"./activity.csv"
	activityNA<-read.csv(file,header=TRUE, sep=",")
...
``` {r impute}
# get mean steps for eagh interval
...
``` {r newdata}
imputeData<-select(imputedData, interval,date,steps)
#names(imputedData$steps.y)<-"steps"
